...
public class SessionServiceImpl implements SessionService {
    ...
    public void saveSession(String userId, String jti, SessionDetails sessionDetails) {
        String key = "session:" + userId + ":" + jti;
        redisTemplate.opsForValue().set(key, sessionDetails, 7, TimeUnit.DAYS);
    }

    @Override
    public void revokeSession(String userId, String jti) {
        redisTemplate.delete("session:" + userId + ":" + jti);
    }
    ...
    public boolean isValidSession(String userId, String jti) {
        return Boolean.TRUE.equals(redisTemplate.hasKey("session:" + userId + ":" + jti));
    }

    @Override
    public SessionDetails getSession(String userId, String jti) {
        return objectMapper.convertValue(
                redisTemplate.opsForValue().get("session:" + userId + ":" + jti),
                SessionDetails.class
        );
    }
